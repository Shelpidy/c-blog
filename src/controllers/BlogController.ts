...
///////////////// GET ALL BLOGS ORDERED BY RANDOM ////////////////////////////
        ...
        let returnPosts = await Promise.all(
            posts.map(async (post) => {
                ...
                let { data: statusData, status: chatResponseStatus } =
                    await axios.get(
                        `http://192.168.1.98:8080/user-status/proxy/${post.getDataValue(
                            "userId"
                        )}`,
                        {
                            headers: {
            ...
            comments.map(async (comment) => {
                ...
                let liked = likes.rows.some(
                    (like) =>
                        ...
                        comment.getDataValue("userId")
                );

                let { data: statusData, status: chatResponseStatus } =
                    await axios.get(
                        `http://192.168.1.98:8080/user-status/proxy/${comment.getDataValue(
                            "userId"
                        )}`,
                        {
                            headers: {
...
app.post("/blogs/:blogId/comments/", async (req, res) => {
    try {
        ...
        let liked = likes.rows.some(
            (like) =>
                like.getDataValue("userId") === comment.getDataValue("userId")
        );
...
app.delete(
    ...
    async (req: express.Request, res: express.Response) => {
        ...
        try {
            ...
            console.log(err);
            res.status(responseStatusCode.BAD_REQUEST).json(
                getResponseBody(responseStatus.ERROR, "", {
                    message: String(err),
                })
            );
        }
...
app.post("/comments/:commentId/replies", async (req, res) => {
    try {
        ...
        let liked = likes.rows.some(
            (like) =>
                like.getDataValue("userId") === reply.getDataValue("userId")
        );

        let { data: statusData, status: chatResponseStatus } = await axios.get(
            `http://192.168.1.98:8080/user-status/proxy/${reply.getDataValue(
                "userId"
            )}`,
            {
                headers: { Authorization: `Bearer ${res.locals.token}` },
...
app.get("/bloggers", async (req: express.Request, res: express.Response) => {
    try {
        const { pageNumber = 1, numberOfRecords = 100 } = req.query;
        let numRecs = Number(numberOfRecords);
        let start = (Number(pageNumber) - 1) * numRecs;
            ...
            getResponseBody(responseStatus.SUCCESS, "", {
                data: await Promise.all(
                    users.map(async (user) => {
                        let { data: statusData, status: chatResponseStatus } =
                            await axios.get(
                                `http://127.0.0.1:8080/user-status/proxy/${user.getDataValue(
                                    "userId"
                                )}`,
                                {
                                    headers: {
