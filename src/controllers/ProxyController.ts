...
router.get("/search",async(req:Request,res:Response)=>{
    try{
      ...
      const blogs = await Blog.findAll({
        where: {
          [Op.or]: [
            ...
            { userId: { [Op.in]: users.map(user => user.getDataValue("userId")) } }
          ],
        },
      });
      ...
      let newBlogs = await Promise.all(
        blogs.map(async (blog) => {
            ...
            let {data:statusData,status:chatResponseStatus} = await axios.get(
                `http://192.168.1.98:8080/user-status/proxy/${blog.getDataValue("userId")}`,{
      ...
      res.status(responseStatusCode.OK).json({ status: responseStatus.SUCCESS, users:newUsers, blogs:newBlogs});
    } catch (error) {
      console.error(error);
